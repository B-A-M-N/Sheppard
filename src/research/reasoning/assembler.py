...
"""
    ...
    async def build_evidence_packet(self, topic_id: str, topic_name: str, section: SectionPlan) -> EvidencePacket:
        ...
        # We use the Hybrid Retriever to pull atoms relevant to the section's purpose
        query_text = f"{topic_name} {section.title} {' '.join(section.target_evidence_roles)}"
        q = RetrievalQuery(text=query_text, topic_filter=topic_id, max_results=15)
        
        # Pull standard atoms
        retrieved_context = await self.retriever.retrieve(q)
        
        # Deduplicate and extract the raw atom dictionaries
        seen_ids = set()
        for item in retrieved_context:
            if item.metadata and item.metadata.get('id'):
                aid = item.metadata['id']
