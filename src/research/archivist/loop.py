...
async def fill_data_gaps(state, section_title, section_goal, chroma_store: ChromaSemanticStore, browser_manager=None):
    """
    Hunter-Seeker Routine: Aggressively finds missing data by broadening the search.
    """
    logger.info(f"[Hunter-Seeker] Triggered for: {section_title}")
...
async def execute_section_cycle(state, section_index, chroma_store: ChromaSemanticStore, is_patching=False, patch_instruction=None, browser_manager=None):
    ...
    for q in queries[:3]:
        try:
            ...
            for u in urls:
                ...
                is_authoritative = any(x in u_l for x in [
                    '.edu', '.gov', 'arxiv.org', 'nature.com', 'science.org',
                    'nytimes.com', 'reuters.com', 'apnews.com', 'bbc.com', 'theguardian.com', 'wsj.com',
                    'documentcloud.org', 'courtlistener.com', 'archives.gov', 'justice.gov', 'fbi.gov',
                    'propublica.org', 'intercept.com', 'newyorker.com', 'theatlantic.com'
                ])
    ...
    raw_docs = await retriever.search(q_emb, top_k=50)
    ...
    if "Specific empirical data for this sub-topic was not found" in section_content:
        ...
        await fill_data_gaps(state, title, goal, chroma_store, browser_manager=browser_manager)

        q_emb = embeddings.get_embedding(f"{state.objective} {title} experimental data")
        raw_docs = await retriever.search(q_emb, top_k=60)
        diverse_docs = []
        for d in raw_docs:
            src_url = d['metadata']['source']
            sid = state.source_registry.get(src_url)
...
async def run_research(
    objective: str,
    chroma_store: ChromaSemanticStore,
    memory_manager=None,
    ollama_client=None,
    browser_manager=None,
    topic=None
) -> dict:
    # Initialize archivist components with the injected store
    index.init(chroma_store)
    retriever.init(chroma_store)
    ...
    for i in range(len(state.plan)):
        await execute_section_cycle(state, i, chroma_store, browser_manager=browser_manager)
    ...
    if audit.get('needs_more_info'):
        ...
        for m in missing:
            ...
            if best_section_idx != -1:
                ...
                await execute_section_cycle(state, best_section_idx, chroma_store, is_patching=True, patch_instruction=m, browser_manager=browser_manager)
