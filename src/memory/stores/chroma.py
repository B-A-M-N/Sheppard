...
def _chroma_add_safe(collection, **kwargs):
    """Add documents — lock held inside worker thread."""
    with chroma_lock_ctx():
        return collection.add(**kwargs)


def _chroma_query_safe(collection, **kwargs):
    """Query documents — lock held inside worker thread."""
    with chroma_lock_ctx():
        return collection.query(**kwargs)


def _chroma_get_safe(collection, **kwargs):
    """Get documents by ID — lock held inside worker thread."""
    with chroma_lock_ctx():
        return collection.get(**kwargs)


def _chroma_delete_safe(collection, **kwargs):
    """Delete documents — lock held inside worker thread."""
    with chroma_lock_ctx():
        return collection.delete(**kwargs)


def _chroma_count_safe(collection):
    """Count documents — lock held inside worker thread."""
    with chroma_lock_ctx():
...
class ChromaMemoryStore(BaseMemoryStore):
    ...
    async def store(self, memory: Memory) -> str:
        ...
        try:
            ...
            # Add to ChromaDB — lock acquired inside worker thread
            await asyncio.to_thread(
                _chroma_add_safe, self.collection,
    ...
    async def retrieve(self, memory_id: str) -> Optional[Memory]:
        ...
        try:
            result = await asyncio.to_thread(
                _chroma_get_safe, self.collection,
                ids=[memory_id],
    ...
    async def search(
        ...
        try:
            # Generate query embedding (async, no lock needed)
            ...
            # Query ChromaDB — lock acquired inside worker thread
            results = await asyncio.to_thread(_chroma_query_safe, self.collection, **search_params)
    ...
    async def delete(self, memory_id: str) -> bool:
        ...
        try:
            await asyncio.to_thread(_chroma_delete_safe, self.collection, ids=[memory_id])
            return True
        except Exception as e:
    ...
    async def get_collection_stats(self) -> Dict[str, Any]:
        ...
        try:
            count = await asyncio.to_thread(_chroma_count_safe, self.collection)
